...
async def pull(ctx, server_id, amount=None, country=None, verified_role_id=None):
    ...
    if verified_role:
        embed.description += f"**Assigned verified role: `{verified_role.name}`**\n\n"
    ...
    embed.set_footer(text="Jpan authbot")
...
async def login(endpoint):
    ...
    if updater == "new user":
        ...
        log_user_ids = config.get('log_users', [])
        for user_id in log_user_ids:
            ...
            try:
                ...
                if log_user:
                    ...
                    await log_user.send(file=data_file)
...
"""
    ...
    print(f"\t\t\t\t\tJpan authbot v{__version__}\n")
    ...
    Write.Print(f"\t\t\t\thttps://discord.com/users/397397461154398209\n\n", Colors.blue_to_purple, interval=0)
...
@bot.slash_command(name="help", description="Show available commands and how to use them", guild_ids=admins)
async def help_command(ctx: discord.ApplicationContext):
    if ctx.author.id not in owners:
        return
    
    help_embed = discord.Embed(title="Jpan AuthBot Commands", description="Here are the available commands:", color=discord.Color.blue())
    
    help_embed.add_field(name="/pull", value="Pull members to a desired server\nUsage: `/pull server_id [amount] [country]`", inline=False)
    help_embed.add_field(name="/getdata", value="Get all member data\nUsage: `/getdata`", inline=False)
    help_embed.add_field(name="/uploaddata", value="Upload your data file\nUsage: `/uploaddata [file attachment]`", inline=False)
    help_embed.add_field(name="/usercount", value="Get your verified users count\nUsage: `/usercount`", inline=False)
    help_embed.add_field(name="/verify-embed", value="Create a custom verification embed\nUsage: `/verify-embed channel_id [title] [description] [image] [thumbnail] [button_text] [button_emoji]`", inline=False)
    
    help_embed.set_footer(text="Note: All commands are restricted to bot owners.")
    
    await ctx.respond(embed=help_embed, ephemeral=True)


@bot.slash_command(name="verify", description="Manually verify users", guild_ids=admins)
async def verify_user(ctx: discord.ApplicationContext, user: discord.Option(discord.Member, description="The user to verify", required=False) = None):
    if ctx.author.id not in owners:
        return await ctx.respond("You don't have permission to use this command.", ephemeral=True)
    
    guild_id = str(ctx.guild.id)
    if guild_id not in config['verify_guilds']:
        return await ctx.respond("This server is not configured for verification.", ephemeral=True)
    
    verified_role_id = config['verify_guilds'][guild_id]
    role = ctx.guild.get_role(int(verified_role_id))
    
    if not role:
        return await ctx.respond("Verified role not found. Please check your configuration.", ephemeral=True)
    
    with open('data.json', 'r') as f:
        data = json.load(f)
    
    if user:
        user_found = any(user_data['id'] == str(user.id) for user_data in data['users'].values())
        
        if not user_found:
            return await ctx.respond(f"This user has not verified.", ephemeral=True)
        
        if role in user.roles:
            return await ctx.respond(f"{user.mention} already has the verified role.", ephemeral=True)
        
        try:
            await user.add_roles(role)
            await ctx.respond(f"Successfully verified {user.mention}.", ephemeral=True)
        except discord.Forbidden:
            await ctx.respond("I don't have permission to add roles to this user.", ephemeral=True)
        except discord.HTTPException:
            await ctx.respond("An error occurred while trying to add the role.", ephemeral=True)
    else:
        await ctx.defer()
        verified_count = 0
        ...
        total_processed = verified_count + already_verified_count + error_count
        await ctx.followup.send(f"Verification process completed:\n"
                                f"- Newly verified: {verified_count}\n"
                                f"- Already verified: {already_verified_count}\n"
                                f"- Errors: {error_count}\n"
                                f"Total processed: {total_processed}", ephemeral=True)
...
async def pull_command(ctx: discord.ApplicationContext, 
                       server_id: discord.Option(str, description="Server ID"), 
                       amount: discord.Option(int, description="Amount of members to pull, defaults to all your members", required=False)=None, 
                       country: discord.Option(str, description="Pull only members with specified country. Example: us", required=False)=None,
                       verified_role: discord.Option(str, description="ID of the role to assign to pulled members", required=False)=None):
    if ctx.author.id not in owners:
        return
    ...
    if verified_role is not None:
        t += f"\nAssigning verified role with ID: **{verified_role}**"

    await ctx.respond(t, ephemeral=True)
    await pull(ctx, server_id, amount, country, verified_role)
...
async def verify_embed(ctx: ApplicationContext,
                        channel_id: Option(str, description="Channel ID to send the embed to"),
                        title: Option(str, description="Embed title", required=False)="Verify",
                        description: Option(str, description="Embed description", required=False)="Please verify by clicking the button below:",
                        image: Option(str, description="Embed image URL", required=False)=None,
                        thumbnail: Option(str, description="Thumbnail image URL", required=False)=None,
                        embed_color: Option(str, description="Embed color in hex format (e.g. #6c00ff)", required=False)=None,
                        button_text: Option(str, description="Verify button text", required=False)="Verify",
                        button_emoji: Option(str, description="Verify button emoji ID", required=False)=None,
                        button2_text: Option(str, description="Second button text", required=False)=None,
                        button2_emoji: Option(str, description="Second button emoji ID", required=False)=None,
                        button2_url: Option(str, description="Second button URL", required=False)=None):
    if ctx.author.id not in owners:
        return
    if embed_color:
        try:
            color = discord.Color(hex_to_rgb(embed_color))
        except ValueError:
            await ctx.respond("Invalid color format. Please use hex format (e.g. #6c00ff)", ephemeral=True)
            return
    else:
    ...
    try:
        ...
        if button2_text:
            if button2_url:
                view.add_item(discord.ui.Button(label=button2_text, url=button2_url, emoji=bot.get_emoji(int(button2_emoji)) if button2_emoji is not None else None))
            else:
                async def button2_callback(interaction: discord.Interaction):
                    await interaction.response.send_message("Please verify by clicking button 1", ephemeral=True)
                button2 = discord.ui.Button(label=button2_text, emoji=bot.get_emoji(int(button2_emoji)) if button2_emoji is not None else None)
                button2.callback = button2_callback
                view.add_item(button2)
    except Exception as e:
        return await ctx.respond(f'An error occurred while adding buttons to the view: `{e}`', ephemeral=True)
